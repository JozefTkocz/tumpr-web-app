...
export type Hill = HillDescription &
  ClassificationsTable & { distance: number; bearing: number } & {
    Number: number
  } & { isBagged: boolean }
...
function useHillData({
  ...
  const { data: baggedIds, markAsBagged, markAsNotBagged } = useBaggedStatus()
  ...
  const distances: Array<SortPosition> | undefined =
    location &&
